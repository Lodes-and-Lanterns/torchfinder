...
// src/state.ts
var DATA_URL = "/dist/torchfinder-dataset.jsonl?v=d2f35298";
var PAGE_SIZE = 25;
...
// src/filter-sidebar-utilities.ts
var FILTER_TOP_N = 8;
...
function buildExpandedHtml(entry, upcoming) {
  ...
  const crowdfundingPlatforms = /* @__PURE__ */ new Set(["kickstarter", "backerkit"]);
  const linksHtml = (entry.links || []).map((link) => {
    ...
    const campaignEndedLabel = isCrowdfunding && entry.date && !upcoming ? ` <span class="badge campaign-ended">Campaign Ended</span>` : "";
    return `<li><a href="${escapeHtml(link.url)}" target="_blank" rel="noopener">${escapeHtml(link.title)}</a>${typeLabel}${pricingLabel}${langLabel}${campaignEndedLabel}</li>`;
  }).join("");
  let parentHtml = "";
...
function init() {
  ...
  showLoading();
  state.data = [];
  const worker = new Worker(new URL("worker.js?v=ffc4e208", import.meta.url), {
    type: "module"
  });
